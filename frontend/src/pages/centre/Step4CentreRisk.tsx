...
export function Step4CentreRisk({
  ...
  return (
    <div className="p-6 md:p-10 lg:p-12 max-w-[1540px] mx-auto w-full space-y-10 select-none font-sans">
      ...
      <section className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pt-2 pb-2">
        <div className="space-y-3 max-w-[850px]">
          ...
          <p className="text-base sm:text-lg text-on-surface-variant leading-relaxed max-w-[800px]">
            Real continuous GBDT regression and classification predicting wastage probability and degradation velocity across the <strong className="text-on-surface font-semibold">200 km Chennai network</strong>.
          </p>
        </div>
      ...
      {/* Risk Band Selectors */}
      <section className="flex flex-wrap items-center gap-2 bg-white p-2.5 rounded-2xl border border-outline-variant/15 text-xs font-bold shadow-2xs">
        {[
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                >
                  <div className="space-y-1.5 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="px-2 py-0.5 bg-surface-container-low text-on-surface text-[10px] font-bold rounded-md font-mono">
                        {item.unit_id}
                      </span>
        ...
        <div className="lg:col-span-5 bg-white p-6 sm:p-8 rounded-3xl border border-outline-variant/15 shadow-xs space-y-6 sticky top-24">
          ...
          {selectedRisk ? (
            <div className="space-y-5 text-xs">
              ...
              <div className="p-4 bg-surface-container-low rounded-2xl border border-outline-variant/15 text-xs text-on-surface leading-relaxed">
                <span className="font-bold text-primary block mb-1">Clinical AI Assessment:</span>
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div className="p-3 bg-surface-container-low/50 rounded-xl space-y-0.5">
                    <span className="text-on-surface-variant block">Remaining Shelf Life</span>
                    <span className="font-bold text-on-surface font-mono text-sm">{selectedRisk.features?.remaining_shelf_life_hours ?? 48} hrs</span>
                  </div>
                  <div className="p-3 bg-surface-container-low/50 rounded-xl space-y-0.5">
                    <span className="text-on-surface-variant block">Current Stock</span>
                    <span className="font-bold text-on-surface font-mono text-sm">{selectedRisk.features?.current_stock ?? selectedRisk.quantity} Units</span>
                  </div>
                  <div className="p-3 bg-surface-container-low/50 rounded-xl space-y-0.5">
                    <span className="text-on-surface-variant block">Demand 24h</span>
                    <span className="font-bold text-on-surface font-mono text-sm">{selectedRisk.features?.demand_next_24h ?? 6} Units</span>
                  </div>
                  <div className="p-3 bg-surface-container-low/50 rounded-xl space-y-0.5">
                    <span className="text-on-surface-variant block">Temp Exposure</span>
                    <span className="font-bold text-secondary font-mono text-sm">{selectedRisk.features?.max_temperature_exposure ?? 22.0}°C</span>
                  </div>
                  <div className="p-3 bg-surface-container-low/50 rounded-xl space-y-0.5">
                    <span className="text-on-surface-variant block">Health Score</span>
                    <span className="font-bold text-on-surface font-mono text-sm">{selectedRisk.features?.health_score ?? 95.0}%</span>
                  </div>
                  <div className="p-3 bg-surface-container-low/50 rounded-xl space-y-0.5">
                    <span className="text-on-surface-variant block">Wastage Score</span>
                    <span className="font-bold text-primary font-mono text-sm">{selectedRisk.features?.wastage_risk_score ?? selectedRisk.risk_score}</span>
                  </div>
                </div>
              </div>
